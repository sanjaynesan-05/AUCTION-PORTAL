...
export default function ViewerScreen() {
  ...
  const handleQuickBid = async (increment: number) => {
    ...
    try {
      ...
      await placeBid(authenticatedTeam.id, bidAmount);
      setBidMessage({type: 'success', text: `Bid placed: ₹${bidAmount}L`});
    } catch (error) {
      setBidMessage({type: 'error', text: 'Failed to place bid'});
    }
  };

  const handleStartFirstBid = async () => {
    if (!authenticatedTeam || !currentPlayer) {
      setBidMessage({type: 'error', text: 'No authenticated team'});
      return;
    }
    try {
      ...
      await placeBid(authenticatedTeam.id, currentPlayer.basePrice);
      setBidMessage({type: 'success', text: `Opening bid: ₹${currentPlayer.basePrice}L`});
    } catch (error) {
      setBidMessage({type: 'error', text: 'Failed to place bid'});
    }
  };
